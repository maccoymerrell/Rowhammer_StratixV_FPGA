...
#include <asm/arch-fsl-lsch3/ls2080a_stream_id.h>
...
#define CONFIG_SYS_PCIE_MEM_SIZE	0xe0000000	/*extended BAR size 1G */
...
#define CONFIG_EXTRA_ENV_SETTINGS		\
	...
	"mcinitcmd=fsl_mc start mc 0x580120010 0x580100010\0"

#ifdef CONFIG_ISSD
#define CONFIG_BOOTARGS		"console=ttyS0,115200 root=/dev/mtdblock9 ro rootfstype=squashfs " \
				"earlycon=uart8250,mmio,0x21c0500,115200 " \
				"default_hugepagesz=2m hugepagesz=2m hugepages=16 modprobe.blacklist=nvme bootimg=issd"

#define CONFIG_BOOTARGS_ISSD_SA		"console=ttyS0,115200 root=/dev/mtdblock9 ro rootfstype=squashfs " \
				"earlycon=uart8250,mmio,0x21c0500,115200 " \
				"default_hugepagesz=2m hugepagesz=2m hugepages=16 modprobe.blacklist=nvme bootimg=issd-sa"

#define CONFIG_BOOTARGS_INIC  	"console=ttyS0,115200 root=/dev/mtdblock9 ro rootfstype=squashfs " \
				"earlycon=uart8250,mmio,0x21c0500,115200 " \
				"default_hugepagesz=2m hugepagesz=2m hugepages=16 modprobe.blacklist=nvme bootimg=inic cma=256m"
#else
...
#ifdef CONFIG_ISSD
#define CONFIG_BOOTCOMMAND "fsl_mc start mc 0x580120010 0x580100010 && " \
                    "fsl_mc apply dpl 0x5800E0000 && " \
					"cp.b $kernel_start_issd $kernel_load $kernel_size  && "     \
					"cp.b $dtb_start $dtb_load $dtb_size && bootm $kernel_load - $dtb_load"

#define CONFIG_BOOTCOMMAND_INIC	"fsl_mc start mc 0x580120010 0x580100010 && " \
                    "fsl_mc apply dpl 0x5800E0000 && " \
					"cp.b $kernel_start_inic $kernel_load $kernel_size  && "     \
					"cp.b $dtb_start $dtb_load $dtb_size && bootm $kernel_load - $dtb_load"

#define CONFIG_BOOTCOMMAND_ISSD	"fsl_mc start mc 0x580120010 0x580100010 && " \
                    "fsl_mc apply dpl 0x5800E0000 && " \
					"cp.b $kernel_start_issd $kernel_load $kernel_size  && "     \
					"cp.b $dtb_start $dtb_load $dtb_size && bootm $kernel_load - $dtb_load"
